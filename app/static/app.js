...
function squareChart(cells, title, centerText){ const corners={0:'corner-tl',3:'corner-tr',9:'corner-bl',12:'corner-br'}; let html = `<div class="panel traditional-chart-panel"><div class="panel-head"><h3>${esc(title)}</h3><div class="micro">ශ්‍රී ලංකා සාම්ප්‍රදායික ආකෘතිය</div></div><div class="square-chart sri-lanka-chart">`; chartOrder.forEach((item,pos)=>{ if(item === 'center'){ html += `<div class="chart-center traditional-center"><div><strong>${esc(centerText)}</strong><span>${esc(title)}</span><em>ශ්‍රී ලංකා සාම්ප්‍රදායික කේන්දර සටහන</em></div></div>`; } else { html += chartBox(cells[item], corners[pos]||''); } }); html += `</div><div class="chart-legend">ග්‍රහ නාම කෙටි අක්ෂරයෙන් • ℞ = වක්‍ර ගමන</div></div>`; return html; }
...
function reportChart(cells, title){ const corners={0:'r-corner-tl',3:'r-corner-tr',9:'r-corner-bl',12:'r-corner-br'}; const box = (cell,corner='')=>`<div class="r-box ${corner}"><div class="r-sign">${esc(cell.sign)} <span>${cell.sign_index+1}</span></div><div class="r-occ">${cell.planets.length ? cell.planets.map(x=>esc(shortPlanetLabel(x))).join('<br>') : ''}</div></div>`; let html = `<div class="report-chart-block"><h4>${esc(title)}</h4><div class="report-chart sri-report-chart">`; chartOrder.forEach((item,pos)=>{ if(item === 'center') html += `<div class="r-center"><strong>${esc(title)}</strong><small>ශ්‍රී ලංකා සාම්ප්‍රදායික ආකෘතිය</small></div>`; else html += box(cells[item],corners[pos]||''); }); html += `</div></div>`; return html; }
