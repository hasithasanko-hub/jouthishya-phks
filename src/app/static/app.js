...
function chartBox(cell){ return `<div class="chart-box"><div class="sign-no">${cell.sign_index + 1}</div><div class="sign-name">${esc(cell.sign)}</div><div class="occupants">${cell.planets.length ? cell.planets.map(esc).join('<br>') : '—'}</div></div>`; }
function squareChart(cells, title, centerText){ let html = `<div class="panel"><div class="panel-head"><h3>${esc(title)}</h3><div class="micro">කොටු 12</div></div><div class="square-chart">`; chartOrder.forEach(item=>{ if(item === 'center'){ html += `<div class="chart-center"><div><strong>${esc(centerText)}</strong><span>ශ්‍රී ලාංකික කොටු කේන්දර ආකෘතිය</span></div></div>`; } else { html += chartBox(cells[item]); } }); html += `</div></div>`; return html; }
...
function reportChart(cells, title){ const box = (cell)=>`<div class="r-box"><div class="r-sign">${esc(cell.sign)} <span>${cell.sign_index+1}</span></div><div class="r-occ">${cell.planets.length ? cell.planets.map(esc).join('<br>') : '—'}</div></div>`; let html = `<div class="report-chart-block"><h4>${esc(title)}</h4><div class="report-chart">`; chartOrder.forEach(item=>{ if(item === 'center') html += `<div class="r-center"><strong>${esc(title)}</strong><small>ශ්‍රී ලාංකික ආකෘතිය</small></div>`; else html += box(cells[item]); }); html += `</div></div>`; return html; }
function openReportWindow(data){
  ...
  const styles = `<style>
    ...
    *{box-sizing:border-box} @page{size:A4;margin:12mm 13mm 14mm}
    ...
    .two{display:grid;grid-template-columns:1fr 1fr;gap:6mm}.report-chart{display:grid;grid-template-columns:repeat(4,1fr);gap:2mm}.r-box,.r-center{border:1px solid #e5d4be;border-radius:8px;padding:2.4mm;min-height:27mm;background:#fffdf9}.r-center{grid-column:2 / span 2;grid-row:2 / span 2;display:grid;place-items:center;text-align:center;background:linear-gradient(180deg,#fff4e9,#fff)}.r-center strong{font-size:14px;color:var(--wine)}.r-center small{display:block;color:#8b7d7f;font-size:8px;margin-top:1mm}.r-sign{font-size:9.5px;font-weight:900;color:var(--wine)}.r-sign span{float:right;color:#b29360}.r-occ{margin-top:1.5mm;font-size:8.7px;line-height:1.45}.report-chart-block h4{margin:0 0 3mm;color:var(--wine);font-size:13px}.houses-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:3mm;margin-top:6mm}.house{border:1px solid var(--line);border-radius:10px;padding:3mm;background:#fffdf9}.house b{font-size:10px;color:var(--wine)}.house small{display:block;font-size:8.5px;color:#817477;margin:1mm 0}.pill{display:inline-block;padding:1mm 2mm;border-radius:999px;background:#f4eadb;font-size:8px;margin:1px}
